...
class TestProdLot(common.SavepointCase):
    ...
    def test_00_sale_stock_invoice_product_lot(self):
        ...
        saleorder = self.env['sale.order.line'].browse(inv_id)
        self.assertEqual(len(saleorder.invoice_line_ids), 2)
        line = saleorder.invoice_line_ids.filtered(
        ...
        # Check S/N
        line2 = saleorder.invoice_line_ids - line
    ...
    def test_01_sale_stock_delivery_partial_invoice_product_lot(self):
        ...
        inv_id = self.sale.action_invoice_create()
        saleorder = self.env['sale.order.line'].browse(inv_id)
        self.assertEqual(len(saleorder.invoice_line_ids), 1)
        line = saleorder.invoice_line_ids
